...
razona de forma composicional. Usando el motor OpenCLIP del Cuaderno 10 calculo los tres
...
evidenciando que el éxito en retrieval no garantiza comprensión composicional. La brecha se
interpreta con los conceptos de fusión profunda (C5) y atención crossmodal (C8).
...
añade que una accuracy mayor no define un modelo "mejor": el diseño de clases y prompts introduce
...
mecanismo se comporta como *bag-of-concepts* y es insensible al orden/relación, justo lo que
...
estándar (R@K) no detecta el problema porque la galería es semánticamente diversa; por eso las
métricas automáticas son necesarias pero **no suficientes**.

**Limitaciones:** el group score mezcla fallo composicional con dificultad/ambigüedad del ítem
(Diwan et al.); el set curado es OOD para CLIP; las comparaciones son estrictas y sensibles a
empates. **Mejora futura:** evaluar un cross-encoder real (C5) y re-ranking con interacción
cruzada (C6); ampliar prompts/checkpoints; versionar embeddings (DVC).

## 5. Conclusión

El retrieval alto no implica razonamiento composicional: OpenCLIP lo ilustra con group score ≈
azar pese a Recall@K alto. El aporte reproducible es el pipeline que mide y contrasta ambos en el
